describe('User e2e scenarios', () => {
  ...
  const authAs = (email: string) => {
    ...
    cy.window().then((win: any) => {
      const headerEl = win.document.querySelector('app-header');
      const ng = win.ng;
      const header = ng?.getComponent?.(headerEl);
      header?.userService?.login(email);
    });
  };
  ...
  it('login shows error for unknown user', () => {
    loginViaUi('unknown@example.com');

    cy.location('pathname').should('eq', '/login');
    cy.contains('Error: LOGIN.ERROR_USER_NOT_FOUND').should('be.visible');
  });
  ...
  it('student sees access denied on course creation', () => {
    ...
    cy.contains('Access denied. Only teacher can create a course.').should('be.visible');
  ...
  it('student can switch profile language to RU', () => {
    ...
    cy.window().should((win: any) => {
      const profileEl = win.document.querySelector('app-profile');
      const ng = win.ng;
      const profile = ng?.getComponent?.(profileEl);
      expect(profile?.i18n?.currentLang).to.equal('ru');
    });
  });
  ...
  it('unknown route opens 404 page and allows return to home', () => {
    ...
    cy.contains('h1, h2, h3', 'Page not found').should('be.visible');
